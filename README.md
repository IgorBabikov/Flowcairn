...
**Нужны:** Node.js 22, существующий проект и доступ к AI. Docker не нужен, чтобы создать задачу, получить план и выполнить AI-этапы. Для безопасного запуска scripts непроверенного проекта нужен Docker. [Платформы и ограничения](docs/INSTALLATION.md).
...
Для безопасных автоматических проверок выберите Docker-режим и подготовьте образ. Он используется в профиле с `checkMode: "hardened"`:
...
`trusted-local` доступен только для известного вам проекта: он запускает exact scripts с правами вашей учетной записи и требует отдельного подтверждения. Не выбирайте его для чужого или непроверенного кода.
...
Неизвестные действия и разрешения блокируются. Задача не может передать произвольную shell-команду, а scripts проекта по умолчанию вообще не запускаются. Docker-режим изолирует зарегистрированные проверки; локальный запуск доступен только после отдельного доверительного согласия. Flowcairn не заменяет проверку списка изменений человеком и не отменяет правила вашей организации или AI-провайдера. Полные границы описаны в [SECURITY.md](SECURITY.md) и [ограничениях](docs/LIMITATIONS.md).
