<p align="center">
  <img src="docs/assets/flowcairn-banner.svg" alt="Flowcairn — большая задача становится управляемой работой." width="100%">
</p>
...
**Нужны:** Node.js 22, существующий проект и доступ к AI. Docker для обычной работы не нужен. Подтвержденные менеджеры: npm, pnpm и Yarn 4 с `node_modules`. [Платформы и ограничения](docs/INSTALLATION.md).
...
По умолчанию проверки выполняются в отдельной рабочей копии проекта через его существующие `typecheck`, `lint`, `test` и `build`. Это дает простой старт без Docker, но не является системной песочницей: запускается код самого проекта и его зависимостей.

Если команде нужен дополнительный изолированный Docker-режим, подготовьте образ отдельно. Он используется только в профиле с `checkMode: "hardened"`:

```bash
npx flowcairn checks prepare
```

Команда может загрузить Docker-образ и зависимости. Она не запускает AI и не нужна для обычного старта.
...
Если в проекте есть стандартные Skills для контекста, планирования, тестирования или ревью, Flowcairn подключает до четырех подходящих файлов сам и показывает их в плане. Неизвестные Skills не активируются автоматически.
...
Неизвестные действия и разрешения блокируются. Задача не может передать произвольную shell-команду, запись ограничена утвержденными путями, а проверки запускают только заранее найденные scripts проекта. Обычный режим использует отдельный worktree; Docker-режим добавляет контейнерную изоляцию. Flowcairn не заменяет проверку списка изменений человеком и не отменяет правила вашей организации или AI-провайдера. Полные границы описаны в [SECURITY.md](SECURITY.md) и [ограничениях](docs/LIMITATIONS.md).
