...
Если проверка не выполнена или результат изменился после нее, требование снова становится неподтвержденным. Flowcairn покажет **«результат подтвержден»** только тогда, когда проверены все обязательные требования и не осталось блокирующих замечаний.
...
Нужны **Node.js 22**, существующий проект с `package.json` и настроенный локальный AI-клиент. Git можно использовать, но для работы Flowcairn он не обязателен. На macOS поддерживается Codex CLI; доступность Claude Code и Cursor зависит от установленного и авторизованного клиента. [Платформы и ограничения](docs/INSTALLATION.md).
...
Перед записью Flowcairn показывает план и ждет вашего подтверждения. По умолчанию он меняет файлы **текущего проекта**, где установлен пакет; отдельный Git worktree можно выбрать явно. [Ограничения текущего выпуска](docs/LIMITATIONS.md).

Подтверждение относится к записанным требованиям и выбранным способам проверки. Оно не означает, что проверены все возможные сценарии или выполнен выпуск в production. Коммит, отправка в Git, публикация и развертывание остаются отдельными действиями.
