...
| --- | --- | --- |
| Codex | Managed npm-runtime Flowcairn, обязательные параметры CLI, capability-проверка и локальная авторизация | Исходный проект, native filesystem permissions для чтения, структурированные предложения правок; применение остается за Executor |
| Claude Code | Managed пакет `@anthropic-ai/claude-code`, допустимая версия от `2.1.198`, capability-проверка и статус авторизации | Исходный проект; Read/Grep/Glob, штатные настройки разрешений, режим dontAsk; MCP и сохранение сессии отключены |
