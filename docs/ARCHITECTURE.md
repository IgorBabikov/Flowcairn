...
flowchart TD
  ...
  W --> K[Registered checks в worktree или Docker]
  K --> R[AI review]
...
Для большого существующего файла AI может вернуть только проверяемое перемещение: исходный путь, целевой путь и hash исходного файла. Runtime убеждается, что оба пути входят в утвержденный scope, исходник не изменился, целевой файл отсутствует и в дереве нет ссылок, затем выполняет локальный rename. Содержимое такого файла не проходит через AI-output. Большое переписывание или слияние JSON остается отдельной подзадачей: Flowcairn не обрезает его молча ради прохождения лимита.

Checks выполняются только через зарегистрированные scripts профиля. По умолчанию runtime запускает их в отдельной Graph worktree с очищенным окружением; Docker остается дополнительным усиленным адаптером. Runtime не берет shell-команду из task JSON. Worktree отделяет изменения от основной копии, но не является системной песочницей: код проекта и зависимости следует считать доверенными.
...
Core/domain Skills остаются в пакете. Init сам подключает до четырех project Skills с понятными ролями контекста, планирования, тестирования и ревью; неизвестные остаются отключенными. Runtime закрепляет исходный текст и эффективные инструкции хешами, передает их AI и проверяет неизменность. [Правила выбора и тестирования](SKILLS.md).

Интеграция Codex, Claude Code и Cursor описана отдельными native manifests, но не равна исполнению AI-node. Только adapter, прошедший ограничения контекста, прав, structured output и receipt, может стать provider Graph Runtime. [Матрица интеграции](AI-HARNESS-INTEGRATION.md).
