...
function fixture(t) {
  ...
  const pkg = (directory) => write(`${directory}/package.json`, JSON.stringify({ name: directory }));
...
test('pnpm YAML packages are authoritative without package.json workspaces', (t) => {
  ...
  assert.deepEqual(discoverWorkspaceManifests(root, {}, 'pnpm'), ['packages/a/package.json', 'packages/b/package.json']);
  assert.deepEqual(discoverWorkspaceManifests(root, {workspaces: ['other/*']}, 'pnpm'), ['packages/a/package.json', 'packages/b/package.json']);
});

test('supports nested, brace, dot-directory patterns and explicit exclusions', (t) => {
  const { root, write, pkg } = fixture(t);
  for (const directory of ['packages/a', 'packages/skip', 'apps/deep/web', 'apps/.hidden/web', 'other/no']) pkg(directory);
  write('pnpm-workspace.yaml', 'packages:\n  - "{packages,apps}/**"\n  - "!packages/skip"\n');
  assert.deepEqual(discoverWorkspaceManifests(root, {}, 'pnpm'), ['apps/.hidden/web/package.json', 'apps/deep/web/package.json', 'packages/a/package.json']);
});
...
test('npm accepts both workspace array and packages object, returns children only', (t) => {
  ...
  pkg('modules/one');
  pkg('modules/two');
  assert.deepEqual(discoverWorkspaceManifests(root, { workspaces: ['modules/*', '!modules/two'] }, 'npm'), ['modules/one/package.json']);
  assert.deepEqual(discoverWorkspaceManifests(root, { workspaces: { packages: ['modules/*'] } }, 'npm'), ['modules/one/package.json', 'modules/two/package.json']);
...
test('Git-tracked and untracked visible manifests are deduplicated without index changes', (t) => {
  ...
  assert.deepEqual(discoverWorkspaceManifests(root, {workspaces: ['packages/*']}, 'npm'), ['packages/tracked/package.json', 'packages/untracked/package.json']);
...
test('workspace patterns reject escaping, absolute, control and oversized forms', (t) => {
  ...
  for (const pattern of ['../outside', '/outside', 'C:/outside', 'packages/../outside', '{../outside,packages}/*', 'packages/\nfoo', 'packages\\foo', 'x'.repeat(513)]) {
    assert.throws(() => discoverWorkspaceManifests(root, {workspaces: [pattern]}, 'npm'), (error) => error.code.startsWith('WORKSPACES_'));
  }
});
...
test('workspace directory and manifest symlinks are rejected', (t) => {
  ...
  assert.throws(() => discoverWorkspaceManifests(root, {workspaces: ['packages/*']}, 'npm'), {code: 'WORKSPACES_FILE'});
...
test('matched ignored manifests fail; explicit exclusions and dependencies stay excluded', (t) => {
  ...
  assert.throws(() => discoverWorkspaceManifests(root, {workspaces: ['**']}, 'npm'), {code: 'WORKSPACES_IGNORED'});
  assert.deepEqual(discoverWorkspaceManifests(root, {workspaces: ['**', '!packages/ignored']}, 'npm'), ['packages/visible/package.json']);
});
