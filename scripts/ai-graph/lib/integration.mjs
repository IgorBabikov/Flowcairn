...
Задачи Flowcairn выполняются через Graph: анализ → редактируемый план → одно согласование → реализация, проверки и ревью в пределах утвержденного плана → готово к личному ревью. Commit и PR выполняет пользователь. Graph не отменяет ограничения host и не разрешает обход sandbox.
...
Настройки проекта определяют добавление новых тестов и coverage: процент покрытия не навязывается. Проверка работоспособности, честные результаты checks и независимое ревью сохраняются.
